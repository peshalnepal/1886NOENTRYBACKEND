...
class ChannelRepository:
    ...
    async def upsert_camera_from_channel_config(
        ...
        *,
        ...
        cam_uuid =cam_uuid or d.get("camera_uuid") or d.get("camera_id")  # support both names
        ...
        if cam:
            ...
            if d.get("camera_uuid") or cam_uuid:
                # Handle both UUID object and string
                cam.camera_uuid = cam_uuid if isinstance(cam_uuid, uuid.UUID) else uuid.UUID(str(cam_uuid))
            db.add(cam)
            try:
    ...
    async def list_cameras(
        ...
        return (await db.execute(select(Camera).where(Camera.site_uuid == site_uuid))).scalars().all()
    ...
    async def _get_camera_by_uuid(self, db: AsyncSession, camera_uuid: uuid.UUID) -> Optional[Camera]:
        from sqlalchemy.orm import selectinload
        stmt = select(Camera).where(Camera.camera_uuid == camera_uuid).options(selectinload(Camera.channel_configuration))
