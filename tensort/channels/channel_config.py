...
class VideoChannelConfig(object):
    """
    Minimal config for video ingest on Jetson. No Pydantic, no external deps.
    ...
    Notes:
      ...
      - ``**kwargs`` is swallowed on purpose: the cloud posts a wider camera
        payload than the edge consumes, and an unknown key must not 500 the
        add/patch route.
    """
